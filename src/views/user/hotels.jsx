...
function Hotels() {
  const { idH } =useParams();
  const {hotel, fetchHotel, lal, imga, fetchImagesHotels} =useDataContext();
  ...
  return (
    <div>
      <Nav />
      <TopTitle lugar={hotel.nombre} imagen={imga} desc={hotel.direccion}/>
      ...
      <div className='contenido-hotels'>
        ...
        <div className='cont-info-hotels'>
          ...
          <div className='dat'>
          <FaPhone /><p>{hotel.telefono}</p>
          </div>
          <div className='dat'>
          <FaEnvelope /><p>{hotel.email}</p>
          </div>
        </div>
      </div>
