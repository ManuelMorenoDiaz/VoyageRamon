...
function DashboardHoteles() {
    ...
    const deleteHotel = async (_id) => {
        try {
            const response = await axios.delete(`http://localhost:3000/hotels/${_id}`);
            if (response.status === 200) {
                Swal.fire({
    ...
    return (
        <>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginRight: '35px' }}>
                <button className='form_add_datos' onClick={handleUsuarioClick} >Agregar</button>
            </div>
            <ModalFormHoteles show={showModal} onClose={closeModal} fetchApi={fetchApi} />
                    ...
                    <tbody className='estilos_tbody'>
                        {Array.isArray(list) ? (
                            list.map((hotel, index) => {
                                const lugar = places.find(place => place._id === hotel.lugar_id);
                                return (
                                    <tr key={index}>
                                        <td className='style-td'>{hotel._id}</td>
                                        <td className='style-td'>{lugar ? lugar.nombre : 'NO tiene lugar'}</td>
